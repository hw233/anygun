...
type COM_ADDiscountStore struct {
	sinceStamp_ uint64                       //0
	endStamp_   uint64                       //1
	contents_   []COM_ADDiscountStoreContent //2
}

func (this *COM_ADDiscountStore) Serialize(buffer *bytes.Buffer) error {
	//field mask
...
func (this *COM_ADDiscountStore) Deserialize(buffer *bytes.Buffer) error {
	//field mask
	...
	if mask.ReadBit() {
		...
		this.contents_ = make([]COM_ADDiscountStoreContent, size)
