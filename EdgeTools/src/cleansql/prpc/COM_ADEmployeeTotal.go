...
type COM_ADEmployeeTotal struct {
	outputNum_  int32                        //0
	sinceStamp_ uint64                       //1
	endStamp_   uint64                       //2
	contents_   []COM_ADEmployeeTotalContent //3
}

func (this *COM_ADEmployeeTotal) Serialize(buffer *bytes.Buffer) error {
	//field mask
	mask := prpc.NewMask1(1)
	mask.WriteBit(this.outputNum_ != 0)
	...
	// serialize outputNum_
	{
		if this.outputNum_ != 0 {
			err := prpc.Write(buffer, this.outputNum_)
			if err != nil {
				return err
			}
		}
...
func (this *COM_ADEmployeeTotal) Deserialize(buffer *bytes.Buffer) error {
	//field mask
	...
	if mask.ReadBit() {
		err := prpc.Read(buffer, &this.outputNum_)
		if err != nil {
			return err
		}
	}
	...
	if mask.ReadBit() {
		...
		this.contents_ = make([]COM_ADEmployeeTotalContent, size)
