...
type COM_ADEmployeeTotalContent struct {
	outputCount_ uint32   //0
	itemIds_     []uint32 //1
	itemStacks_  []uint32 //2
	status_      uint8    //3
}

func (this *COM_ADEmployeeTotalContent) Serialize(buffer *bytes.Buffer) error {
	//field mask
	mask := prpc.NewMask1(1)
	mask.WriteBit(this.outputCount_ != 0)
	...
	// serialize outputCount_
	{
		if this.outputCount_ != 0 {
			err := prpc.Write(buffer, this.outputCount_)
			if err != nil {
				return err
			}
		}
...
func (this *COM_ADEmployeeTotalContent) Deserialize(buffer *bytes.Buffer) error {
	//field mask
	...
	if mask.ReadBit() {
		err := prpc.Read(buffer, &this.outputCount_)
		if err != nil {
			return err
		}
	}
