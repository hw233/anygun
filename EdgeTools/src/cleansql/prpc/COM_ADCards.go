...
type COM_ADCards struct {
	sinceStamp_ uint64               //0
	endStamp_   uint64               //1
	contents_   []COM_ADCardsContent //2
}

func (this *COM_ADCards) Serialize(buffer *bytes.Buffer) error {
	//field mask
...
func (this *COM_ADCards) Deserialize(buffer *bytes.Buffer) error {
	//field mask
	...
	if mask.ReadBit() {
		...
		this.contents_ = make([]COM_ADCardsContent, size)
