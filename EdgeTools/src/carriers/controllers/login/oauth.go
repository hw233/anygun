...
func (c *Oauth) Get() {
	...
	if oauthObj.Status == "ok" {
		...
		if len(servs) == 0 {
			beego.Error("Channel =======> ", channel, " can not find servs")
			if b {
		...
		for i := 0; i < len(servs); i++ {
			host := models.GetServOauthHostById(servs[i].Id)
			go func(s string) {
				//beego.Debug("Connect game server " + s)
				gamereq := httplib.Post(s)
				gamereq.Body([]byte(str))
				gamereq.SetTimeout(time.Second, time.Second)
