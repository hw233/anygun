...
#define __COPY_PROPERTIES(TO,FR,ID) (TO).properties_[ID] = (FR).properties_[ID];
	...
	__COPY_PROPERTIES(inst,(*tmp),PT_NoSleep);			///抗昏睡
	__COPY_PROPERTIES(inst,(*tmp),PT_NoPetrifaction);	///抗石化
	__COPY_PROPERTIES(inst,(*tmp),PT_NoDrunk);			///抗酒醉
	__COPY_PROPERTIES(inst,(*tmp),PT_NoChaos);			///抗混乱
	__COPY_PROPERTIES(inst,(*tmp),PT_NoForget);			///抗遗忘
	__COPY_PROPERTIES(inst,(*tmp),PT_NoPoison);			///抗毒
	__COPY_PROPERTIES(inst,(*tmp),PT_Assassinate);		///暗杀
	
	__COPY_PROPERTIES(inst,(*this),PT_Money);
	__COPY_PROPERTIES(inst,(*this),PT_Diamond);
	__COPY_PROPERTIES(inst,(*this),PT_MagicCurrency);			//魔力币
	__COPY_PROPERTIES(inst,(*this),PT_EmployeeCurrency);		//伙伴回收点
	__COPY_PROPERTIES(inst,(*this),PT_Gather);   //采集挖矿消耗
	__COPY_PROPERTIES(inst,(*this),PT_Level);       ///等级  
	__COPY_PROPERTIES(inst,(*this),PT_Exp);         ///经验
	__COPY_PROPERTIES(inst,(*this),PT_ConvertExp);	///活动转储经验
	__COPY_PROPERTIES(inst,(*this),PT_OneDayReputation); ///一天可获得最大声望
	__COPY_PROPERTIES(inst,(*this),PT_Reputation);  ///声望
	__COPY_PROPERTIES(inst,(*this),PT_TableId);     ///资源表索引
	__COPY_PROPERTIES(inst,(*this),PT_AssetId);		///主显示资源
	__COPY_PROPERTIES(inst,(*this),PT_Sex);			    ///性别
	__COPY_PROPERTIES(inst,(*this),PT_BagNum);    ///开启背包槽位数
	__COPY_PROPERTIES(inst,(*this),PT_Race);		///种族
	__COPY_PROPERTIES(inst,(*this),PT_Profession);	///职业
	__COPY_PROPERTIES(inst,(*this),PT_ProfessionLevel); ///职业等级
	///当前称号
	__COPY_PROPERTIES(inst,(*this),PT_Title); 
	__COPY_PROPERTIES(inst,(*this),PT_GuildID);
	__COPY_PROPERTIES(inst,(*this),PT_Glamour); ///魅力值
	__COPY_PROPERTIES(inst,(*this),PT_DoubleExp);	//双倍经验
	__COPY_PROPERTIES(inst,(*this),PT_TongjiComplateTimes); ///统计完成计数 个人
	__COPY_PROPERTIES(inst,(*this),PT_VipLevel); ///VIP 等级
	__COPY_PROPERTIES(inst,(*this),PT_VipTime);		///vip时间

	__COPY_PROPERTIES(inst,(*this),PT_Stama); 		///体力 一级属性
	__COPY_PROPERTIES(inst,(*this),PT_Strength);	///力量
	__COPY_PROPERTIES(inst,(*this),PT_Power); 		///强度
	__COPY_PROPERTIES(inst,(*this),PT_Speed); 	 	///速度
	__COPY_PROPERTIES(inst,(*this),PT_Magic);	 	///魔法
	__COPY_PROPERTIES(inst,(*tmp),PT_Durability);	///耐久 	

	__COPY_PROPERTIES(inst,(*tmp),PT_HpMax);		///生命值
	__COPY_PROPERTIES(inst,(*tmp),PT_MpMax);		///魔法值
	__COPY_PROPERTIES(inst,(*tmp),PT_Attack);		///攻击力
	__COPY_PROPERTIES(inst,(*tmp),PT_Defense);		///防御力
	__COPY_PROPERTIES(inst,(*tmp),PT_Agile);		///敏捷
	inst.properties_[PT_Spirit] = inst.properties_[PT_Reply] = 100;
	__COPY_PROPERTIES(inst,(*tmp),PT_Magicattack);	///魔攻
	__COPY_PROPERTIES(inst,(*tmp),PT_Magicdefense);///抗魔
	__COPY_PROPERTIES(inst,(*tmp),PT_Damage);		///伤害
	__COPY_PROPERTIES(inst,(*tmp),PT_SneakAttack);	///偷袭属性
	__COPY_PROPERTIES(inst,(*tmp),PT_Hit);				///命中  修正属性
	__COPY_PROPERTIES(inst,(*tmp),PT_Dodge);			///闪躲
	__COPY_PROPERTIES(inst,(*tmp),PT_Crit);			///必杀
	__COPY_PROPERTIES(inst,(*tmp),PT_counterpunch);	///反击
	__COPY_PROPERTIES(inst,(*tmp),PT_Front);		///是否前排 0后排 1前排
	__COPY_PROPERTIES(inst,(*tmp),PT_Wind);		///风 水晶属性
	__COPY_PROPERTIES(inst,(*tmp),PT_Land);		///地
	__COPY_PROPERTIES(inst,(*tmp),PT_Water);		///水
	__COPY_PROPERTIES(inst,(*tmp),PT_Fire);		///火
	__COPY_PROPERTIES(inst,(*tmp),PT_Free);		//自由点数
	__COPY_PROPERTIES(inst,(*tmp),PT_FightingForce); ///战斗力
	__COPY_PROPERTIES(inst,(*this),PT_Free); ///未加的属性点
...
void Player::passZeroHour()
{
	...
	if(PeriodEvent::getCurWeek() == Global::get<int>(C_CleanRandQuestWeekDay)){
		cleanRandQuestCounter(); ///如果是周一 清除
	}
...
Player::save()
{
	...
	logoutTime_ = WorldServ::instance()->curTime_; //五分钟更新一次logout 时间
...
Player::genItemInst(U32 itemId, U32 itemCount, std::vector<COM_Item*> &items,bool isBind, bool isLock )
{
	...
	while(itemCount > 0)
	{
		...
		else if(core->mainType_ == IMT_BabyEquip){ ///宝宝装备随机技能和技能等级
...
COM_Item*
Player::getBagItemMinStackByItemId(U32 itemId) ///最小堆叠
{
	std::vector<COM_Item*> items;
	getBagItemByItemId(itemId,items);
	if(items.empty())
...
bool Player::wearEquipment(U32 target,U32 itemInstId)
{
	...
	if(item->mainType_ == IMT_Equip ){
		...
		if(item->subType_ >= IST_Axe && item->subType_ <=IST_Knife ){
			if(item->slot_ == ES_SingleHand){ //装备单手武器
				if(equipItems_[ES_DoubleHand] != NULL){
					...
					if(item2->subType_ != IST_Shield){ //这家伙不是盾
						//CALL_CLIENT(this,wearEquipmentOk(0,*current));
						errorMessageToC(EN_BagFull);
						return true;
					}
...
Player::setPlayerInst(COM_PlayerInst &tmp)
{
	...
	myselfrecharge_ = tmp.myselfRecharge_ ; //个人累计充值常驻版
	selfRecharge_ = tmp.selfRecharge_ ; //个人累计充值
	sysRecharge_ = tmp.sysRecharge_ ; //系统累计充值
	selfDiscountStore_ = tmp.selfDiscountStore_; //打折商
	sysDiscountStore_ = tmp.sysDiscountStore_ ; //打折商
	selfOnceRecharge_ = tmp.selfOnceRecharge_ ; ///单笔充值
...
Player::getPlayerInst(COM_PlayerInst &out)
{
	...
	out.myselfRecharge_ = myselfrecharge_ ; //个人累计充值常驻版
	out.selfRecharge_ = selfRecharge_ ; //个人累计充值
	out.sysRecharge_ = sysRecharge_ ; //系统累计充值
	out.selfDiscountStore_ = selfDiscountStore_; //打折商
	out.sysDiscountStore_ = sysDiscountStore_ ; //打折商
	out.selfOnceRecharge_ = selfOnceRecharge_ ; ///单笔充值
...
Player::setDBPlayerData(SGE_DBPlayerData &tmp)
{
	...
	firstRollEmployeeCon_ = tmp.firstRollEmployeeCon_; //第一次金币roll
	firstRollEmployeeDia_ = tmp.firstRollEmployeeDia_ ; //第一次钻石roll
...
Player::getDBPlayerData(SGE_DBPlayerData &out)
{
	...
	out.magicTupoLevel_ = magicTupoLevel_;
	out.firstRollEmployeeCon_ = firstRollEmployeeCon_; //第一次金币roll
	out.firstRollEmployeeDia_ = firstRollEmployeeDia_ ; //第一次钻石roll
...
{
	Battle * battle = Battle::find(battleId_); ///这是个bug battleId 没有清
	if(NULL == battle)
	{
...
Player::addProperty(U32 guid,const std::vector<COM_Addprop> &props)
{
	...
	for(size_t k=0;k<inst.properties_.size(); ++k){
		setProp((PropertyType)k,inst.properties_[k]); //设置新属性
	}
...
Player::forgetSkill(S32 skId)
{
	...
	Entity::forgetSkill(skId); //这里只是删除
...
bool Player::giveDrop(S32 dropId,bool usebase){
	...
	// 日常任务获得奖励增加一个系数
	//dropId 220000-230000 日常任务dropId drop->exp_ 特殊处理
	//任务等级=【四舍五入(PLAYERLEVEL/10)】*10-5
	// 系数=(PLAYERLEVEL+1所对应的经验-LV所对应的经验）/（任务等级+1所对应的等级经验-任务等级所对应的经验）
	//EXP = drop->exp_*系数
	if(drop->exp_){
...
Player::resetBaby(S32 babyId)
{
	...
	if(NULL == tmp)
		return ;
	if(tmp->monsterType_ == 3)	//3为改造宠物不能还原
		return;
...
void Player::talkedNpc(int npcId)
{
	...
	if(npcd->npcType_ == NT_Mogu || npcd->npcType_ == NT_Xiji || npcd->npcType_ == NT_SinglePK || npcd->npcType_ == NT_TeamPK){
		...
		if (getBagEmptySlot() <= 0){
			errorMessageToC(EN_BagFull); // 采集背包满了不能talk
			return ;
		}
	}
	talkNpc(npcId);
...
Player::syncMyJJCTeamMsg()
{
	...
	if(leader != this)	//不是队长,队伍中有暂离队员，不同步
	{
...
Player::sell(S32 iteminstid, S32 babyinstid,S32 price){
	...
	if(item){
		...
		if(NULL == p)
			return;
		sellitem.baby_.instName_  = p->name_; ///售出时宠物名恢复默认
...
void Player::sellOk(COM_SellItem item){
	...
	addMoney(-Global::get<float>(C_MallSellPay)); //扣去摊位费
...
void Player::buy(S32 guid){
	...
	if(waitSell_)
		return;
	if(getProp(PT_VipLevel) < VL_2){
		CALL_CLIENT(this,errorno(EN_NotSuperVip)); //不是超级VIP
		return;  
	}
	SGE_BuyContent bc;
...
void Player::fixEquipment(S32 instId, FixType type)
{
	...
	enum{		
		DURA_MoneyRatio = 10, //金钱休息衰减系数
		DURA_MoneyRatioMin = 60 , //金钱修理最小系数
		DURA_MoneyPay = 50,
	};
...
void Player::wearFuwen(int32 itemInstId){
	...
	GEParam param[ARG_MAX];
	param[ARG0].type_  = GEP_INT;
	param[ARG0].value_.i = alllevel;
	GameEvent::procGameEvent(GET_WearFuwen,param,ARG_MAX,handleId_);
}
...
void Player::compFuwen(int32 itemInstId){
	...
	if(pItem->stack_ > Global::get<int>(C_CompRunesNeedNum)){
		...
		delBagItemByInstId(pItem->instId_,pItem->stack_,18); ///这句话之后就不能使用pItem
