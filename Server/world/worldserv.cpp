...
#include "GameScript.h"
	...
	std::string mainLua =std::string(DESIGN_SCRIPT_ROOT"main.lua");
	...
	if(false == loadPeriodEvent(DESIGN_TABLE_ROOT"PeriodEvent.xml"))
		SRV_ASSERT(0);
	
	if(false == TableSystem::instance()->Load())
		SRV_ASSERT(0);

	if(false == Activity::load(DESIGN_TABLE_ROOT"DailyActivities.csv"))
		SRV_ASSERT(0);
	if(false == Zhuanpan::load(DESIGN_TABLE_ROOT"ZhuanpanConfig.csv"))
		SRV_ASSERT(0);
	if(false == Activity::loadrewards(DESIGN_TABLE_ROOT"ACT_Reward.csv"))
		SRV_ASSERT(0);
	if(false == ExamTable::load(DESIGN_TABLE_ROOT"question.csv"))
		SRV_ASSERT(0);
	if(false == Employeeoutput::qualityload(DESIGN_TABLE_ROOT"QualityWeight.csv"))
		SRV_ASSERT(0);
	if(false == Employeeoutput::load(DESIGN_TABLE_ROOT"EmployeeWeight.csv"))
		SRV_ASSERT(0);

	if(false == Festival::load(DESIGN_TABLE_ROOT"festival.csv"))
		SRV_ASSERT(0);
	if(false == RechargeTotal::load(DESIGN_TABLE_ROOT"recharge.csv"))
		SRV_ASSERT(0);
	if(false == RechargeSingle::load(DESIGN_TABLE_ROOT"recharge_single.csv"))
		SRV_ASSERT(0);
	if(false == DiscountStore::load(DESIGN_TABLE_ROOT"sale.csv"))
		SRV_ASSERT(0);
	if(false == ReversalCard::loadcardsper(DESIGN_TABLE_ROOT"cardsper.csv"))
		SRV_ASSERT(0);
	if(false == ReversalCard::loadcardreward(DESIGN_TABLE_ROOT"cardsreward.csv"))
		SRV_ASSERT(0);
	if(false == HotShop::load(DESIGN_TABLE_ROOT"hotshop.csv"))
		SRV_ASSERT(0);
	if(false == SevenDayTable::load(DESIGN_TABLE_ROOT"Sevendays.csv"))
		SRV_ASSERT(0);
	if(false == EmployeeActivityTotal::load(DESIGN_TABLE_ROOT"EmployeeActivity.csv"))
		SRV_ASSERT(0);
	if(false == MySelfRecharge::load(DESIGN_TABLE_ROOT"recharge_all.csv"))
		SRV_ASSERT(0);
	if(false == LevelGift::load(DESIGN_TABLE_ROOT"LevelGift.csv"))
		SRV_ASSERT(0);
	if(false == IntegralShop::load(DESIGN_TABLE_ROOT"ScoreShop.csv"))
		SRV_ASSERT(0);
	if(false == CourseGiftTable::load(DESIGN_TABLE_ROOT"CourseGift.csv"))
		SRV_ASSERT(0);
...
int WorldServ::handle_timeout (const ACE_Time_Value &tv, const void *act)
{
	...
	LoginOauth::timeOutOauths(delta);
...
void WorldServ::accept()
{
	...
	/*if(Env::get<int>(V_UsedAnySDK))*/
	{
		static ACE_Acceptor< LoginOauth , ACE_SOCK_ACCEPTOR > acceptor;
		ACE_INET_Addr addr(Env::get<const char*>(V_AnySDKOauthListen));
		if(acceptor.open(addr) == -1)
		{
			ACE_DEBUG((LM_INFO,ACE_TEXT("World serv listen [%s|ANY-SDK LoginOauth] field\n"),Env::get<const char*>(V_AnySDKOauthListen)));
			return;
		}
		ACE_DEBUG((LM_ERROR,ACE_TEXT("World serv listen [%s|LoginOauth] \n"),Env::get<const char*>(V_AnySDKOauthListen)));
	}

	/*if(Env::get<int>(V_UsedAnySDK))*/
	{
		static ACE_Acceptor< PayNotify , ACE_SOCK_ACCEPTOR > acceptor;
		ACE_INET_Addr addr(Env::get<const char*>(V_AnySDKPayNotifyListen));
		if(acceptor.open(addr) == -1)
		{
			ACE_DEBUG((LM_INFO,ACE_TEXT("World serv listen [%s|ANY-SDK PayNotify] field\n"),Env::get<const char*>(V_AnySDKPayNotifyListen)));
			return;
		}
		ACE_DEBUG((LM_ERROR,ACE_TEXT("World serv listen [%s|PayNotify] \n"),Env::get<const char*>(V_AnySDKPayNotifyListen)));
	}
}
