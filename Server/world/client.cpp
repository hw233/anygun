...
ClientHandler::login(COM_LoginInfo& info)
{
	...
	VerifiedAccount va;
	if(Env::get<int32>(V_UsedAnySDK) && !isrobot){
		AnyOauth* pOauth = LoginOauth::getOauthByUsername(info.username_);
		if(pOauth){
			va.sdkInfo_ = *pOauth;
			va.sdkInfo_.idfa_ = info.idfa_;
			va.sdkInfo_.mac_ = info.mac_;
			va.sdkInfo_.deviceType_ = info.devicetype_;
		}else {
			return false;
		}
		//LoginOauth::removeOauthByUsername(info.username_);
	}
