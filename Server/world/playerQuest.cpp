...
Player::submitQuest(S32 npcId, S32 questId, int32 instId)
{
	...
	if(pDrop != NULL){
		...
		if(pDrop->items_.size() > getBagEmptySlot()){//2141 【需求】背包位置不足的情况下，对于任务的一些处理
			...
			GameEvent::procGameEvent(GET_BagFullSendMail,param,ARG_MAX,handleId_);
		}
		else
			giveDrop(tmpdropid);
	}
	...
	if(quest->questKind_ == QK_Profession && quest->jt_ != 0 && quest->jl_ != 0){
		...
		GameEvent::procGameEvent(GET_ChangeProfession,param,ARG_MAX,handleId_);
		...
		GameEvent::procGameEvent(GET_Richang,param,ARG_MAX,handleId_);
	}
	else if(quest->questKind_ == QK_Tongji){
		enum {
			ARG0,
		...
		param[ARG0].type_  = GEP_INT;
		param[ARG0].value_.i = 1;
		GameEvent::procGameEvent(GET_Tongji,param,ARG_MAX,handleId_);
	}
